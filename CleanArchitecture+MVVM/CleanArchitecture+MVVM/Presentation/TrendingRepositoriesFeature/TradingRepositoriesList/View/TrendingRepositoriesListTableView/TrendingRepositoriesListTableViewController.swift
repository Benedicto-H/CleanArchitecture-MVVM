...
final class TrendingRepositoriesListTableViewController: UITableViewController {
    ...
    private func updateSeparatorStyle() -> Void {
        
        switch viewModel.content.value {
        case .emptyData: tableView.separatorStyle = .none
        case .items, .loading: tableView.separatorStyle = .singleLine
        }
    }
    ...
    private func updateItems() -> Void {
        ...
        switch viewModel.content.value {
        case .items(let models): snapshot.appendItems(models.map { .item($0) }, toSection: .main)
        case .emptyData: snapshot.appendItems([.emptyData], toSection: .main)
        case .loading:
            let loadingItems: [Cell] = (0..<Constants.numberOfLoadingShimmeringCells).map { .loading(index: $0) }
...
extension TrendingRepositoriesListTableViewController {
    private func dequeueReusableCell(_ tableView: UITableView, cell: Cell) -> UITableViewCell {
        
        switch cell {
        case .item(let item):
            let cell = tableView.dequeueReusableCell(TrendingRepositoriesListItemCell.self, registeredCellTypes: &registeredCellTypes)
            
            cell.configure(with: item, imagesRepository: imagesRepository)
            
            return cell
        case .emptyData:
            return tableView.dequeueReusableCell(TrendingRepositoriesListEmptyItemCell.self, registeredCellTypes: &registeredCellTypes)
        case .loading:
            return tableView.dequeueReusableCell(TrendingRepositoriesListLoadingItemCell.self, registeredCellTypes: &registeredCellTypes)
        }
    }
}
...
extension TrendingRepositoriesListTableViewController {
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        switch viewModel.content.value {
        case .emptyData: return tableView.frame.height * 0.8
        case .items, .loading: return super.tableView(tableView, heightForRowAt: indexPath)
        }
    }
